...
export default function Dashboard() {
  ...
  const fetchData = async () => {
    try {
      const [rankRes, salesRes] = await Promise.all([
        fetch('http://localhost:3000/api/dashboard/ranking'),
    ...
    <main className="min-h-screen bg-[#0f172a] text-slate-100 p-4 md:p-8">
      <div className="max-w-7xl mx-auto">
        <header className="mb-10">
          <h1 className="text-3xl font-extrabold">Fluxo Influencer</h1>
          <p className="text-slate-400 font-medium">Gestão La Ganexa</p>
        </header>

        {/* TOP 5 CARDS */}
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-5 gap-4 mb-10">
          {ranking.map((item) => (
            <div key={item.coupon} className="bg-slate-800/50 border border-slate-700 p-5 rounded-2xl shadow-xl">
              <span className={`text-[10px] font-bold px-2 py-0.5 rounded-full ${item.status === 'verde' ? 'bg-emerald-500/10 text-emerald-400' : 'bg-rose-500/10 text-rose-400'}`}>
                ROI {item.status === 'verde' ? '3+' : 'BAIXO'}
              </span>
              <h3 className="text-slate-400 text-sm mt-3 uppercase font-bold">{item.coupon}</h3>
              <p className="text-2xl font-black">R$ {item.total.toLocaleString('pt-BR')}</p>
            </div>
          ))}
        </div>
        ...
        <div className="bg-slate-800/50 border border-slate-700 p-6 rounded-2xl mb-10 shadow-xl">
          <h2 className="text-xl font-bold mb-6 flex items-center"><TrendingUp className="mr-2 text-blue-400"/> Ranking Performance</h2>
            ...
            <ResponsiveContainer width="100%" height="100%">
              <BarChart data={ranking}>
                ...
                <Bar dataKey="total" radius={[4, 4, 0, 0]}>
                  {ranking.map((e, i) => <Cell key={i} fill={e.status === 'verde' ? '#10b981' : '#f43f5e'} />)}
                </Bar>
              </BarChart>
            </ResponsiveContainer>
          </div>
              ...
              <tbody className="divide-y divide-slate-700">
                {recentSales.map((sale) => (
                  <tr key={sale.id} className="hover:bg-slate-700/30">
                    <td className="p-4 text-slate-400 text-xs">#{sale.order_id}</td>
                    <td className="p-4 font-bold text-blue-400">{sale.coupon}</td>
                    <td className="p-4 font-bold">R$ {Number(sale.value).toFixed(2)}</td>
                    <td className="p-4"><div className={`w-3 h-3 rounded-full ${sale.roi_status === 'verde' ? 'bg-emerald-500' : 'bg-rose-500'}`} /></td>
                  </tr>
                ))}
              </tbody>
            </table>
